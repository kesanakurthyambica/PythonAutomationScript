...
def replace_bind_name(xml_file, text_name, new_bind_name, output_file=None):
    ...
    for elem in root.iter():
        if elem.tag == "Group":
            ...
            current_text = elem.attrib.get("Name", "")
        elif elem.tag == "Bind" and in_group and current_text == text_name:
            print(f"Replacing Bind '{elem.attrib.get('Name')}' with '{new_bind_name}' for Text '{text_name}'")
    ...
    print(f"Updated XML saved to: {output}")
 
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python replace_bind.py <xml_file> <text_name> <new_bind_name>")
    else:
        xml_path = sys.argv[1]
        text_to_find = sys.argv[2]
        new_bind = sys.argv[3]
        replace_bind_name(xml_path, text_to_find, new_bind)